...
uint64 sys_sigreturn(void)
{
  ...
  myproc()->alarmlock  =0;  // * open the func
  return 0;
}
...
uint64 sys_sigalarm(void)
{
  ...
  if(argint(0, &ntick) < 0)
    return -1;
  if(argaddr(0, &funcaddr) < 0)
    return -1;
  myproc()->tickset   = 1;  // * open the func
  myproc()->handler   = (void(*)())funcaddr;  // *function handler
  myproc()->set_ticks_interval   = ntick ;  // * ticks interval
  return 0;
}